...
namespace CoffeeShop.Controllers
{
    public class DiagnosticController : Controller
    {
            ...
            IConfiguration configuration)
        {
            ...
            _logger = logger;
            _connectionString = $"Data Source={Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "CoffeeShop.db")}";
        }
        ...
        [HttpGet("test-db-error")]
        public IActionResult TestDbError()
        {
            using var connection = new SqliteConnection(_connectionString);
            connection.Open();
            // Симуляция DB ошибки
            connection.Query("SELECT * FROM NonExistentTable");
            return Ok();
        }
    }
}